...
fun HomeScreen(
    ...
    BoxWithConstraints(modifier = modifier.fillMaxSize()) {
        val homeSectionPadding = if (useTvLayout) 56.dp else homeSectionHorizontalPaddingForWidth(maxWidth.value)
        val homeRowItemSpacing = if (useTvLayout) 22.dp else null
        val homeRowBottomPadding = if (useTvLayout) 24.dp else 12.dp
                    ...
                    if (continueWatchingPreferences.isVisible && continueWatchingItems.isNotEmpty()) {
                        item(key = HOME_CONTINUE_WATCHING_SECTION_KEY) {
                            HomeContinueWatchingSection(
                                ...
                                itemSpacing = homeRowItemSpacing,
                                enableTvFocus = useTvLayout,
                            ...
                            if (section != null && section.items.isNotEmpty()) {
                                item(key = settingsItem.key) {
                                    HomeCatalogRowSection(
                                        ...
                                        onApproachEnd = if (section.nextSkip != null && !section.isLoadingMore) {
                                            { HomeRepository.loadMoreSection(section.key) }
                                        } else {
                                            null
                                        },

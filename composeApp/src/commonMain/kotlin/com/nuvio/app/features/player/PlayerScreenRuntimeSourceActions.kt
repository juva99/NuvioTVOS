...
internal data class PlayerResumePoint(val positionMs: Long, val fraction: Float?)

/**
 * Resume point to carry over to the next source. While playback is live we use the current
 * position, but if the active stream never started (e.g. an expired link that failed to open,
 * leaving the position at 0) we fall back to the furthest known spot so picking a new link
 * continues where the user left off instead of restarting.
 */
internal fun PlayerScreenRuntime.resumePointForSourceSwitch(): PlayerResumePoint {
    val livePositionMs = playbackSnapshot.positionMs.coerceAtLeast(0L)
    if (livePositionMs > 0L) return PlayerResumePoint(livePositionMs, null)
    // The stream produced no live position (it failed to open, or the engine cleared the
    // position on error). Fall back to the furthest known spot: the persisted watch progress
    // first, then the position the player was launched with.
    val savedPositionMs = activeVideoId
        ?.let { WatchProgressRepository.progressForVideo(it) }
        ?.takeIf { !it.isCompleted }
        ?.lastPositionMs
        ?.takeIf { it > 0L }
    return if (savedPositionMs != null) {
        PlayerResumePoint(savedPositionMs, null)
    } else {
        PlayerResumePoint(activeInitialPositionMs.coerceAtLeast(0L), activeInitialProgressFraction)
    }
}
...
internal fun PlayerScreenRuntime.switchToP2pSourceStream(stream: StreamItem) {
    ...
    val resume = resumePointForSourceSwitch()
    flushWatchProgress()
    stopActiveP2pStream()
...
/**
 * Recovery for a fatal playback error (typically an expired/dead stream link): drop the broken
 * link so it isn't reused, keep the saved watch progress, and open the sources panel with fresh
 * results so the user can pick a new link. The resume position is preserved, so the chosen link
 * continues where they left off. Returns false (so the caller falls back to the error overlay)
 * when there is no content to reselect for or the panel is already open.
 */
internal fun PlayerScreenRuntime.offerSourceReselectionAfterError(): Boolean {
    val vid = activeVideoId ?: return false
    if (showSourcesPanel) return false
    removeFailedStreamFromCache()
    errorMessage = null
    openedSourcesPanelAfterError = true
    PlayerStreamsRepository.loadSources(
        type = contentType ?: parentMetaType,
        videoId = vid,
        season = activeSeasonNumber,

...
internal class PlayerScreenRuntime(
    ...
    // True while the sources panel is shown as recovery from a fatal playback error (e.g. an
    // expired link), so dismissing it without picking a new link exits the player instead of
    // returning to the dead stream.
    var openedSourcesPanelAfterError by mutableStateOf(false)

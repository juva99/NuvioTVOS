...
fun isProgressComplete(
    ...
    // `isEnded` must mean a clean end-of-stream from the player engine — an end-of-file caused
    // by an error (e.g. an expired stream link) must not reach here as `isEnded`, otherwise the
    // title would be wrongly marked complete and dropped from Continue Watching. The engine is
    // responsible for that distinction (see the MPV bridge `isPlayerEnded` handling).
